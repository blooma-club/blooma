...
const ImageEditPanel: React.FC<Props> = ({ storyboardId, frameId, currentImageUrl, onImageUpdated, isAutoGenerated = false }) => {
  ...
  const canGenerate = useMemo(() => (references.length > 0 || currentImageUrl) && prompt.trim().length > 0, [references.length, currentImageUrl, prompt]);
  ...
  const handleUpload = async (file: File) => {
    ...
    try {
      ...
      form.append("storyboardId", storyboardId || "empty");
  ...
  const handleRemoveReference = (idx: number) => setReferences((prev) => prev.filter((_, i) => i !== idx));
  const handleChangeReference = (idx: number, url: string) => setReferences((prev) => prev.map((v, i) => (i === idx ? url : v)));
  ...
  const handleGenerate = async () => {
    ...
    try {
      const image_urls = [
        ...
        ...references.filter((u) => u && /^https?:\/\//i.test(u.trim())),
      ];
    ...
    <div className="flex flex-col gap-6 p-4 text-sm">
      ...
      {isAutoGenerated && (
        <div className="text-blue-400 bg-blue-900/30 border border-blue-700 rounded p-3 text-sm">
          💡 This storyboard was auto-generated from a script. You can still upload custom images to replace the AI-generated ones.
        </div>
      )}

             {/* Thumbnail / Upload */}
       <div>
         <div className="text-sm font-semibold tracking-wide text-neutral-300 uppercase mb-3">Image</div>
                   {currentImageUrl ? (
            <div className="relative">
              {imageLoadError ? (
                <div className="w-full h-40 flex flex-col items-center justify-center rounded border border-neutral-700 bg-neutral-800 text-neutral-400">
                  ...
                  <div className="text-[9px] text-neutral-500">URL: {currentImageUrl.slice(0, 50)}...</div>
                  <button
                    type="button"
      ...
      {/* References */}
      <div>
        <h3 className="text-sm font-semibold tracking-wide text-neutral-300 uppercase mb-3">Reference Images</h3>
