...
export const ProjectCard = ({ project, viewMode, onDelete, reverse = false }: ProjectCardProps) => {
  const router = useRouter()
  const [isMenuOpen, setIsMenuOpen] = useState(false)
  const [isLoading] = useState(false)

  const handleGoToProjectSetup = () => router.push(`/project/${project.id}/setup`)
  ...
  const renderGridView = () => (
    <div className="relative group cursor-pointer" onClick={handleGoToProjectSetup}>
      ...
      <div className={cn(
        getCardClass('pt-3 pb-6 px-6')
      )}>
        <div className="flex items-start justify-between mb-4">
          <div className="flex-1 min-w-0">
            <h3 className="text-lg font-semibold text-gray-900 truncate mb-2">{project.title}</h3>
            <p className="text-sm text-gray-500 mt-0.5 line-clamp-2">{project.description || 'No description provided.'}</p>
          </div>
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-4">
              <div className="flex-1">
                <h3 className="text-lg font-semibold text-gray-900 truncate mb-2">{project.title}</h3>
                <p className="text-sm text-gray-500 mt-0.5 truncate">{project.description || 'No description provided.'}</p>
              </div>
              {/* 날짜/공개여부 상하 분리 */}
              <div className="flex flex-col space-y-2 mt-2">
                <div className="flex items-center gap-1">
