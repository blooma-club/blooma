...
>
> **Update Note (2026-02-02)**: Plan/product mapping and `hasActiveSubscription()` live in `src/lib/billing/logic.ts`. Credit consumption lives in `src/lib/billing/credits.ts`. `users.subscription_tier` should use `'free'` (not `null`) when there is no active plan.
...
| `src/lib/billing/logic.ts`              | `hasActiveSubscription()` / 플랜↔Product ID 매핑 | - (설정/읽기 전용)                         |
| `src/lib/db/users.ts`                   | User DB CRUD, 크레딧/구독 업데이트                | `users` 테이블                             |
| `src/lib/billing/credits.ts`            | 크레딧 차감/환불 로직                             | `users.credits_used`                       |
...
sequenceDiagram
    ...
    alt subscription.created/active
        ...
        Webhook->>DB: updateUserSubscriptionTier(userId, 'free')
    end
...
| `subscription_tier`     | ✅       | 'Small Brands'/'Agency'/'Studio'/'free' |
| `credits`               | ✅       | 총 크레딧                               |
| `credits_used`          | ✅       | 사용 크레딧                             |
...
- **위치**: `src/app/api/billing/webhook/route.ts`
- **재현**: 유저가 취소 후 취소 철회 시
- **영향**: `subscription_tier`가 'free'로 유지됨
