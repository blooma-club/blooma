...
- `subscription_tier`: free | Small Brands | Agency | Studio
...
2. 서버가 `src/lib/billing/logic.ts`로 상품 ID 결정 (`getProductIdForPlan`)
3. Polar 체크아웃 URL 반환
...
- 상품 ID 매핑 오류: `src/lib/billing/logic.ts` 및 환경 변수 확인
