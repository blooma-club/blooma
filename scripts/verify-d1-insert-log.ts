...
const logFile = path.resolve(process.cwd(), 'd1-error.log');
...
async function executeD1Request<T>(
    ...
    log(`Executing D1 Query: ${requestPayload.sql}`);
    log(`Params: ${JSON.stringify(requestPayload.params)}`);

    const response = await fetch(endpoint, requestInit)
    const responsePayload = await parseResponse<T>(response)

    if (!response.ok || !responsePayload.success) {
        log('D1 Query Failed Response:');
        log(JSON.stringify(responsePayload, null, 2));
...
async function syncClerkUser(profile: ClerkUserProfile): Promise<D1UserRecord> {
    ...
    if (existingByClerkId) {
        log('User found by Clerk ID, updating...');
    ...
    if (profile.email) {
        ...
        if (existingByEmail) {
            log('User found by Email, updating...');
...
async function main() {
    log('--- Starting D1 Verification (Standalone Insert with Log) ---');
    ...
    try {
        ...
        log(`Attempting to sync user: ${testUserId}`);
        const result = await syncClerkUser(testUser);
        log('✅ Successfully synced user!');
        log('Result: ' + JSON.stringify(result, null, 2));

    } catch (error) {
        log('❌ Failed to sync user.');
        if (error instanceof D1QueryError) {
            log('D1 Query Error Details:');
            // @ts-ignore
